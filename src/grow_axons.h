...
public:
    ...
    std::vector<Dynamic_Sphere> GrowAxon(Axon* ax, Eigen::Vector3d destination, std::vector<Eigen::Vector2d>& all_twin_delta_pos, std::ostream& out);
    ...
    bool find_next_center(Axon* ax, Eigen::Vector3d destination, Dynamic_Sphere& s, std::vector<Eigen::Vector3d> centers, double dist_, double& rad, std::ostream& out, bool& collides_with_border, Eigen::Vector2d& twin_delta_pos,  std::vector<Eigen::Vector2d> phi_theta_colliding, int max_tries);
    bool isSphereColliding(Dynamic_Sphere sph);
    bool fiber_collapse(std::vector<Eigen::Vector3d>& centers, int fibre_collapsed_nbr, std::ostream& out);
